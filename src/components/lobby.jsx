...
const uid = new ShortUniqueId({
  length: window._env_.REACT_APP_ROOM_ID_LENGHT,
});

const Lobby = (props) => {
  const { user, setUser, setChooseName, setJoined, setMe, setRoom, addPlayer } =
    ...
    <>
      ...
      <div className="flex items-center justify-center sm:mx-48">
        <div className="h-404 w-4/12 rounded-2xl drop-shadow-xl p-6 mr-4 bg-dirty-white md:mr-2 max-w-custom-1">
          <div className="h-14">
            <p className="text-center text-dirty-purple font-bold font-roboto">
              ELIGE UN AVATAR Y UN APODO
            </p>
          </div>
          ...
          <form noValidate onSubmit={handleSubmit}>
            <div className="flex justify-center items-center pt-24 relative">
              <input
                className={`border-2 border-dirty-input rounded-10 h-33 w-138 focus:outline-none text-dirty-purple placeholder:text-dirty-ph text-center font-roboto ${
                  error ? "invalid:border-dirty-error" : ""
                }`}
                type="text"
